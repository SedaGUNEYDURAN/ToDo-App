...
•	**JPA(Java Persistence API)**, JAVA uygulamalarında veri kalıcılığınısağlamak için kullanılan bir spesifikasyondur. Bu API, Java'da nesne ile ilişkisel veritabanı yönetim sistemleri(RDBMS) arasındaki haritalama işlemlerini kolaylaştırır. Nesnesel ilişkisel haritalama(ORM) yaparak Java sınıfları ile veritabanı tabloları arasında bir köprü kurar.   
